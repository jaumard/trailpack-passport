          ...
          if (!req.user) {
            ...
            // Action:   Get the user associated with the passport.
              ...
              app.orm.Passport.update(passport.id, passport)
                .then(passport => {
                  app.orm.User.find(passport.user.id, next).catch(err => next(err))
                }).catch(err => next(err))
            }
          }
          else {
  ...
  loadStrategies: (app) => {
    _.each(app.config.session.strategies, (strategiesOptions, name) => {
        ...
        if (name == 'local') {
          ...
          const baseUrl = 'http://localhost:3000' //FIXME find real url
          ...
          passport.use(new Strategy(options, app.services.PassportService.protocols[protocol]));
        }
      }
    )
  }
