        ...
        .then(passport => {
          if (!req.user) {
            ...
            // Action:   Get the user associated with the passport.
              ...
              // I save any update to the Passport before moving on and
              // I read the associated user instance
              return Promise.all([
                ...
                .then(results => {
                  let userInstance = results[0]; // Not so usefull, just for keeping namings clear
                  return app.services.FootprintService.find('user', userInstance.id)
                })
                .then(user => {
                  next(null, user)
                })
