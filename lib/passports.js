...
module.exports = {
  init: (app) => {
    ...
    passport.connect = function (req, query, profile, next) {
      ...
      const provider = query.provider || profile.provider
      ...
      app.services.FootprintService.find('passport', {
        ...
        if (!req.user) {
          ...
          if (passport.length === 0) {
            ...
            app.config.passport.mergeThirdPartyProfile(user, profile).then(mergedProfile =>
              app.services.FootprintService.create('user', mergedProfile).then(user => {
                ...
                query.user = user.id
                return app.services.FootprintService.createAssociation('user', user.id, 'passports', query)
                  .then(passport => next(null, user))
                  .catch(err => {
                    //if we can't register passport object we delete the user
                    return app.services.FootprintService.destroy('user', user.id).then(() => {
          ...
          // Action:   Simply pass along the already established session.
            ...
            const userId = passport.user || passport.UserId // waterline || sequelize
    ...
    passport.serializeUser((user, next) => {
      if (user == null || !user.id) {
        next(new Error('User has not been correctly pass to the session'))
      }
      else {
        next(null, user.id)
      }
