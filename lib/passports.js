...
module.exports = {
  init: (app) => {
        ...
        .then(passport => {
          if (!req.user) {
            ...
            // Action:   Get the user associated with the passport.
             ...
             // TODO: Must be tested for check if ok for both Sequelize and Waterline
             // Save any updates to the Passport before moving on
              app.services.FootprintService.find('Passport', passport.id)
                .then(passport => {
                  // I can't rely on the update results because on trailpack-sequelize return an array with the updated rows counter
                  // So, first I read passport model
                  app.services.FootprintService.update("Passport", passport.id, passport)
                    .then(() => {
                      // I read UserId field (very bad) 
                      return app.services.FootprintService.find('user', passport.UserId)
                    })
                    .then(user => {
                      next(null, user)
                    })
    ...
    passport.deserializeUser((id, next) => {
      app.services.FootprintService.find('user', id, {findOne: true}).then(user => {
        next(null, user)
      }).catch(err => {
        next(err)
      })
